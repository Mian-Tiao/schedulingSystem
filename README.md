...
7. **AI 諮詢與操作**:
   - 以真實排程數據回答「為什麼這個方案第一?」「哪台機台是瓶頸?」等問題。
   - 可用自然語言查詢產品、機台與訂單,取得方案分析,預演急單插入或機台故障。
   - 可要求新增訂單、修改待排程/已排程訂單,或重新執行排程。
   - 所有寫入操作只會先建立確認卡;使用者按下「確認執行」後 Server 才會呼叫共用 Service 寫入資料。
   - 情境模擬為唯讀預演,不會直接寫入正式排程或停機資料。

## 排程模型(MVP 假設)

- 一張訂單一道工序;production 可跨午休、下班、維護與隔日分段續做,setup/cleaning 則不可中斷。
